...
import ToolsDocument from './manager/tools.js';
import HomeDocument from './manager/home.js';
import CreditHtml from './manager/credit.js';
import { 
  rootDomain, 
  serviceName, 
  apiKey, 
  ...
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname.startsWith("/sub")) {
        ...
        const home = new HomeDocument(request, cachedProxyList);
          for (const proxy of cachedProxyList) {
           home.registerProxies(
           { country: proxy.country, countryName: proxy.countryName },
            [proxy],
          );
         }
        const result = home.build();
     ...
     else if (url.pathname.startsWith("/credit")) {
      return new Response(CreditHtml, {
        status: 200,
        headers: { "Content-Type": "text/html;charset=utf-8" },
      });
     }
  ...
  <body >
   ...
   <ul class="flex justify-center space-x-4">
    ...
    <div id="container-window" class="hidden">
      <!-- Windows -->
      <!-- Informations -->
      <div class="fixed z-20 inset-0 w-[75%] h-[50%] bg-white bg-opacity-80 dark:bg-neutral-800 dark:bg-opacity-50 rounded-lg shadow-lg flex justify-center items-center m-auto hidden">


   <p id="container-window-info" class="text-center w-full h-full top-1/4 absolute dark:text-white"></p>
      </div>
      <!-- Output Format -->
      <div id="output-window" class="fixed z-20 inset-0 w-full h-full flex justify-center items-center hidden">
        <div class="w-[90%] md:w-[75%] max-w-2xl h-auto md:h-[30%] flex flex-col gap-3 p-4 text-center bg-white/10 backdrop-blur-lg rounded-xl border border-white/20">
          <div class="w-full rounded-md">
            <div class="flex flex-col md:flex-row w-full gap-3 justify-center">
              <button onclick="copyToClipboardAsTarget('clash')"
                class="w-full md:basis-1/3 p-3 rounded-xl bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 text-white font-medium shadow-lg hover:shadow-blue-500/30 transform hover:-translate-y-0.5 transition-all duration-200 flex justify-center items-center gap-2">
              ...
              <button onclick="copyToClipboardAsTarget('sfa')"
                class="w-full md:basis-1/3 p-3 rounded-xl bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white font-medium shadow-lg hover:shadow-purple-500/30 transform hover:-translate-y-0.5 transition-all duration-200 flex justify-center items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                  <path fill-rule="evenodd" d="M14.615 1.595a.75.75 0 01.359.852L12.982 9.75h7.268a.75.75 0 01.548 1.262l-10.5 11.25a.75.75 0 01-1.272-.71l1.992-7.302H3.75a.75.75 0 01-.548-1.262l10.5-11.25a.75.75 0 01.913-.143z" clip-rule="evenodd" />
                </svg>
                SFA
              </button>
              <button onclick="copyToClipboardAsTarget('bfr')"
                class="w-full md:basis-1/3 p-3 rounded-xl bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white font-medium shadow-lg hover:shadow-green-500/30 transform hover:-translate-y-0.5 transition-all duration-200 flex justify-center items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
          ...
          <div class="w-full rounded-md">
            <div class="flex flex-col md:flex-row w-full gap-3 justify-center">
              <button onclick="copyToClipboardAsTarget('v2ray')"
                class="w-full md:basis-1/3 p-3 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white font-medium shadow-lg hover:shadow-amber-500/30 transform hover:-translate-y-0.5 transition-all duration-200 flex justify-center items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M11.47 1.72a.75.75 0 011.06 0l3 3a.75.75 0 01-1.06 1.06l-1.72-1.72V7.5h-1.5V4.06L9.53 5.78a.75.75 0 01-1.06-1.06l3-3zM11.25 7.5V15a.75.75 0 001.5 0V7.5h3.75a3 3 0 013 3v9a3 3 0 01-3 3h-9a3 3 0 01-3-3v-9a3 3 0 013-3h3.75z" />
                </svg>
                V2Ray/Xray
              </button>
              <button onclick="copyToClipboardAsRaw()"
                class="w-full md:basis-1/3 p-3 rounded-xl bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white font-medium shadow-lg hover:shadow-red-500/30 transform hover:-translate-y-0.5 transition-all duration-200 flex justify-center items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
              ...
              <button onclick="toggleOutputWindow()" 
                class="w-full md:basis-1/3 p-3 rounded-xl bg-gradient-to-r from-gray-500 to-gray-600 hover:from-gray-600 hover:to-gray-700 text-white font-medium shadow-lg hover:shadow-gray-500/30 transform hover:-translate-y-0.5 transition-all duration-200 flex justify-center items-center gap-2">
                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 24 24" fill="currentColor">
...
class Document {
  ...
  buildProxyGroup() {
    ...
    proxyGroupElement += `<div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">`;
    for (let i = 0; i < this.proxies.length; i++) {
      const proxyData = this.proxies[i];

      proxyGroupElement += `
      <div class="proxy-card bg-white/10 backdrop-blur-sm dark:bg-neutral-800/70 
                  transition-transform duration-200 rounded-lg p-3 w-60 border border-white/20 
                  hover:border-blue-500/50 hover:-translate-y-1 relative">
        
        <!-- Country Flag -->
        <div class="absolute -top-3 -left-3">
          <img width="32" class="border border-white/50 rounded-full" 
               src="https://hatscripts.github.io/circle-flags/flags/${proxyData.country.toLowerCase()}.svg" />
        </div>

        <!-- Status -->
        <div class="ml-7">
          <div id="ping-${i}" class="text-xs font-medium text-white/80">
            <span class="inline-block w-2 h-2 rounded-full bg-amber-400 mr-1"></span>
            <span class="truncate">Idle ${proxyData.proxyIP}:${proxyData.proxyPort}</span>
          </div>
        </div>

        <!-- Info Card -->
        <div class="mt-3 p-2 rounded-lg bg-black/10 border border-white/10">
          <h5 class="font-bold text-sm text-white mb-2 truncate">${proxyData.org}</h5>
          <div class="text-sm text-white/80">
            <div class="flex items-center gap-1">
              <span>IP: ${proxyData.proxyIP}</span>
            </div>
            <div class="flex items-center gap-1">
              <span>Port: ${proxyData.proxyPort}</span>
            </div>
            <div id="container-region-check-${i}" class="text-xs text-white/60">
        ...
        // Protocol buttons in pairs
        for (let x = 0; x < proxyData.list.length; x += 2) {
          const protocols = [
            { name: `${reverse("NAJORT")} TLS`, icon: '🔒' },
            { name: `${reverse("SSELV")} TLS`, icon: '🔒' },
            { name: `${reverse("SS")} TLS`, icon: '🔒' },
            { name: `${reverse("NAJORT")} NTLS`, icon: '🔓' },
            { name: `${reverse("SSELV")} NTLS`, icon: '🔓' },
            { name: `${reverse("SS")} NTLS`, icon: '🔓' }
          ];

          proxyGroupElement += `
          <div class="flex gap-1">
            <button onclick="copyToClipboard('${proxyData.list[x]}')"
                    class="flex items-center justify-center gap-1 w-full px-2 py-1
                           bg-blue-500/80 hover:bg-blue-600/80
                           dark:bg-blue-500/20 hover:dark:bg-blue-500/30
                           rounded text-white text-xs font-medium">
              ${protocols[x].icon}
              <span>${protocols[x].name}</span>
            </button>`;

          if (proxyData.list[x + 1]) {
            proxyGroupElement += `
            <button onclick="copyToClipboard('${proxyData.list[x + 1]}')"
                    class="flex items-center justify-center gap-1 w-full px-2 py-1
                           bg-purple-500/80 hover:bg-purple-600/80
                           dark:bg-purple-500/20 hover:dark:bg-purple-500/30
                           rounded text-white text-xs font-medium">
              ${protocols[x + 1].icon}
              <span>${protocols[x + 1].name}</span>
            </button>`;
          }
